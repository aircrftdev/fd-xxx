    ...
    if (numFields === 2) {
        ...
        querySortKeyTypes.forEach((fieldType, index) => {
            if (graphql_transformer_common_1.getBaseType(fieldType) !== graphql_transformer_common_1.getBaseType(tableSortKeyTypes[index])) {
                throw new graphql_transformer_core_1.InvalidDirectiveError(`${querySortFields[index]} field is not of type ${graphql_transformer_common_1.getBaseType(tableSortKeyTypes[index])}`);
            }
        });
    }
}
      ...
      `);
        ...
        this.field = (parent, field, directive, ctx) => {
            ...
            if (leftConnectionIsList && rightConnectionIsList) {
                ...
                if (sortFieldName) {
                    ...
                    if (!sortField) {
                        throw new graphql_transformer_core_1.InvalidDirectiveError(`sortField with name "${sortFieldName} cannot be found on tyoe: ${parent.name.value}`);
                    }
        ...
        this.connectionWithKey = (parent, field, directive, ctx) => {
            ...
            if (!graphql_transformer_common_1.isListType(field.type)) {
                ...
                const queryResolver = this.resources.makeQueryConnectionWithKeyResolver(parentTypeName, fieldName, relatedType, args.fields, keySchema, index ? String(index.IndexName) : undefined);
                ctx.setResource(graphql_transformer_common_2.ResolverResourceIDs.ResolverResourceID(parentTypeName, fieldName), queryResolver);
