    ...
    return {
        ...
        fields: input.fields.reduce((acc, f) => {
            if (keyFields.find(k => k === f.name.value)) {
                return [...acc, graphql_transformer_common_1.makeInputValueDefinition(f.name.value, graphql_transformer_common_1.makeNonNullType(graphql_transformer_common_1.makeNamedType(graphql_transformer_common_1.getBaseType(f.type))))];
            }
            return [...acc, f];
        }, []),
    };
...
function makeQueryResolver(definition, directive, ctx) {
    ...
    return new cloudform_types_1.AppSync.Resolver({
        ...
        RequestMappingTemplate: graphql_mapping_template_1.print(graphql_mapping_template_1.compoundExpression([
            ...
            graphql_mapping_template_1.iff(graphql_mapping_template_1.ref('context.args.nextToken'), graphql_mapping_template_1.set(graphql_mapping_template_1.ref(`${requestVariable}.nextToken`), graphql_mapping_template_1.ref('util.toJson($context.args.nextToken)')), true),
