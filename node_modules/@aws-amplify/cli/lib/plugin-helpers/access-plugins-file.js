...
async function readPluginsJsonFile() {
    let result;
    const pluginsFilePath = getPluginsJsonFilePath();
    const exists = await fs_extra_1.default.pathExists(pluginsFilePath);
    if (exists) {
        result = await readJsonFile_1.readJsonFile(pluginsFilePath);
    }
    return result;
}
...
async function writePluginsJsonFile(pluginsJson) {
    const systemDotAmplifyDirPath = getSystemDotAmplifyDirPath();
    const pluginsJsonFilePath = path_1.default.join(systemDotAmplifyDirPath, getPluginsJsonFileName());
    await fs_extra_1.default.ensureDir(systemDotAmplifyDirPath);
    const jsonString = JSON.stringify(pluginsJson, null, JSON_SPACE);
    await fs_extra_1.default.writeFile(pluginsJsonFilePath, jsonString, 'utf8');
}
